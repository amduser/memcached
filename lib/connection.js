...
issues.log = function log (message) {
  ...
  if (this.failures && this.failures == this.config.failures) {
    this.failuresResetId = issue.setTimeout(
      issue.failuresReset.bind(issue),
      this.failuresTimeout
    );
  }
  ...
  if (this.failuresResetId) this.clearTimeout(this.failuresResetId);
...
issues.attemptReconnect = function attemptReconnect () {
  ...
  ping(this.tokens[1], function pingpong (err) {
    ...
    if (err) {
      ...
      return issue.setTimeout(
        issue.attemptReconnect.bind(issue),
        issue.reconnect
      );
    }
...
issues.setTimeout = function IssueLog_setTimeout (fn, ms) {
  var issue = this;
...
issues.clearTimeout = function IssueLog_clearTimeout (timer) {
  clearTimeout(timer);
...
issues.clearTimers = function clearTimers () {
